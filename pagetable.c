...
int allocate_frame(pgtbl_entry_t *p) {
	...
	if(frame == -1) { // Didn't find a free page.
		...
		//used for ARC evict functions
		coremap[0].last_pointer = p;
		frame = evict_fcn();
		...
		//store victim page and check if it is dirty
		pgtbl_entry_t *victim = coremap[frame].pte;
		if (victim->frame & PG_DIRTY) {
			//check if swap works and swap frame into swapfile
			int swap = swap_pageout(frame, victim->swap_off);
			if (swap == INVALID_SWAP){
				exit(1);
			}
			//change frame format and increment dirty count
			victim->swap_off = swap;
			evict_dirty_count++;
		...
		//get rid of valid bit in victim frame
...
char *find_physpage(addr_t vaddr, char type) {
	...
	//(void)idx; // To keep compiler happy - remove when ready for use
	// IMPLEMENTATION NEEDED
	// Use top-level page directory to get pointer to 2nd-level page table

	//check if second level frame is valid.
	if (!(pgdir[idx].pde & PG_VALID)){
		pgdir[idx] = init_second_level();
	}
	pgtbl_entry_t *pt_2 = (pgtbl_entry_t *)(pgdir[idx].pde & PAGE_MASK);

	// Use vaddr to get index into 2nd-level page table and initialize 'p'
	p = &pt_2[PGTBL_INDEX(vaddr)];

	// Check if p is valid or not, on swap or not, and handle appropriately
	//check if frame is in coremap
	if (p->frame & PG_VALID){
		...
		//check if page frame is in swap file
		if (p->frame & PG_ONSWAP){
			...
			//initialize frame and edit frame bits
